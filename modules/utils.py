...
def findElementByTag(root, name, elem=None):
    for element in root:
        # print(element.tag)
        if '{https://www.gov.pl/static/zagospodarowanieprzestrzenne/schemas/app/2.0}'+name in element.tag:
            ...
            for elem in elements:
                if fe.name == 'mapaPodkladowa':
                    ...
                    objName = (elem.tag.replace(
                        '{https://www.gov.pl/static/zagospodarowanieprzestrzenne/schemas/app/2.0}', ''))+'_lineEdit'
                    objValue = elem.text
